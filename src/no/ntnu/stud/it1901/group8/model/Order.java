...
public class Order {
	...
	private final static TimeZone timeZone = TimeZone
			.getTimeZone("Europe/Oslo");
	private static Calendar calendar = Calendar.getInstance(timeZone, locale);

	/**
	 * Creates a new order with all the information needed. This is used if you
	 * want to edit an existing order, and the order object is not initialized.
	 * 
	 * @param customer
	 * @param products
	 * @param status
	 * @param comment
	 */
	private Order(int idOrder, Customer customer, ArrayList<Product> products,
			String status, String comment, int allergy, int delivery, long date) {
	...
	/**
	 * Creates a new Order with only a customer. More can be added later on. Use
	...
	public double getOrderTotalPrice() {
		...
		if (products.contains(deliveryFee)
				&& (totalprice - DeliveryFee.getOriginalFee()) >= limitFreeDelivery) {
	...
	/**
	 * Updates the order in the database. The method actually deletes the old
	 * one, and insert the new order.
	 */
	public void updateOrder() {
		...
		db.insert("DELETE FROM product_has_order WHERE orders_idorder="
				+ this.idOrder);
		// Adds the order
		String query = "INSERT INTO orders (status, comments, customer_idcustomer, allergy, delivery, time) "
				+ "VALUES ('"
	...
	public void addProductToOrder(Product product, int quantity) {
		...
		if (!(product instanceof DeliveryFee)
				&& productsContainsProduct(product.getName())) {
			for (Product p : products) {
	...
	/**
	 * Checks if the product already exist in the order. You have to check by
	 * name, since the same product can be in different objects.
	 * 
	 * @param name
	 * @return True if the product exist in the order, false else.
	 */
	...
	/**
	 * Removes products from the order. If the order has no other products than
	 * the delivery fee after the removal, the delivery fee is also removed
	 * 
	 * @param product
	 * @param quantity
	 */
	...
	/**
	 * Gets all the orders with two given statuses and order it by time
	 * ascending from the database.
	 * 
	 * @param status1
	 * @param status2
	 * @return ArrayList with orders
	 */
	public static ArrayList<Order> getRelevantOrders(String status1,
			String status2) {
		ArrayList<Order> relevantOrders = new ArrayList<Order>();
		try {
			Database db;
			db = Database.getDatabase();
			ResultSet rs = db.select("SELECT * FROM orders WHERE status='"
					+ status1 + "' OR status='" + status2
					+ "' ORDER BY time ASC");
			while (rs.next()) {
				ArrayList<Product> products = Product.getProductsFromOrder(rs
						.getInt("idorder"));
				Customer customer = Customer.getCustomerFromOrder(rs
						.getInt("customer_idcustomer"));
				...
				relevantOrders.add(new Order(idOrder, customer, products,
						status, comment, allergy, delivery, date));
			}
	...
	public static ArrayList<Order> getAllOrders() {
		...
		try {
			...
			while (rs.next()) {
				...
				int delivery = rs.getInt("delivery");
				long date = rs.getLong("time");
				allOrders.add(new Order(idOrder, customer, products, status,
						comment, allergy, delivery, date));
			}
			return allOrders;
		} catch (SQLException e) {
			Error.databaseError();
	...
	/**
	 * Gets all the orders that are not finished and order it by time ascending
	 * from the database.
	 * 
	...
	public static ArrayList<Order> getOrdersInProcess() {
		...
		try {
			...
			db = Database.getDatabase();
			ResultSet rs = db
					.select("SELECT * FROM orders where status!='Utlevert' and status!='Levert' ORDER BY time ASC");
			while (rs.next()) {
				ArrayList<Product> products = Product.getProductsFromOrder(rs
						.getInt("idorder"));
				Customer customer = Customer.getCustomerFromOrder(rs
	...
	/**
	 * Returns the total price of taxes in the order.
	 * 
	 * @return totalTax
	 */
	public double getTotalTaxInOrder() {
		double totalTax = 0;
		for (Product product : products) {
			totalTax += product.getTax() * product.getQuantity();
		}
		return totalTax;
	}
