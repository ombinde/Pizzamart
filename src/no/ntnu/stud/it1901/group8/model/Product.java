...
public class Product {
	...
	/**
	 * Creates a new Product with the given id, name, price, quantity, comment
	 * and visibility. The constructor will look up the product id in the
	 * database.
	 * 
	 * @param id
	 * @param name
	...
	public Product(int id, String name, double price, int quantity,
		...
		this.tax = price * Properties.taxRate;
	}
	...
	public static ArrayList<Product> getProductsFromOrder(int idOrder) {
		...
		try {
			ResultSet rs = db
					.select("SELECT idproduct, name, price, comments, quantity FROM product_has_order "
							+ "JOIN product ON idproduct=product_idproduct WHERE orders_idorder="
							+ idOrder);
	...
	/**
	 * Returns an ArrayList with the relevant products to the query. The
	...
	public static ArrayList<Product> getRelevantProducts(String query) {
		...
		try {
			Database db = Database.getDatabase();
			ResultSet rs = db.select("SELECT * FROM product where name LIKE '"
					+ query + "%' AND visability=1 ORDER BY name ASC");
			while (rs.next()) {
				int idProduct = rs.getInt("idproduct");
				double price = rs.getDouble("price");
				String name = rs.getString("name");
	...
	/**
	 * Returns an ArrayList with all the products in the database. The products
	...
	public static ArrayList<Product> getAllProducts() {
		...
		try {
			Database db = Database.getDatabase();
			ResultSet rs = db
					.select("SELECT * FROM product WHERE visability=1 ORDER BY name ASC");
			while (rs.next()) {
				int idProduct = rs.getInt("idproduct");
				double price = rs.getDouble("price");
				String name = rs.getString("name");
	...
	/**
	 * Adds a product to the database with the given name and price if the
	 * product does not already exist.
	 * 
	 * @param name
	 * @param price
	 * @return true if the product was successfully added, and false if the
	...
	/**
	 * Changes the visibility to false, so the product should not be shown
	 * anymore, but will exist in existing orders. Returns true if the product
	...
	public static boolean deleteProductFromDatabase(Product p) {
		...
		Database db = Database.getDatabase();
		String query = "UPDATE product SET visability='0' where name='"
				+ p.name + "'";
		db.insert(query);
		return true;
	}
	...
	/**
	 * Updates a product p in the database with the given parameters. Returns
	 * true if the product was updated successfully. If false something wrong
	 * happened, and it wasn't updated.
	 * 
	 * @param p
	 * @param name
	 * @param price
	...
	public static boolean updateProduct(Product p, String name, double price,
		...
		Database db = Database.getDatabase();
		String query = "UPDATE product SET name='" + name + "', price='"
				+ price + "', comments='" + comment + "' WHERE name='" + p.name
				+ "'";
		db.insert(query);
		return true;
	}
	...
	/**
	 * Checks that the product doesn't already exist in the database. Only one
	...
	private static boolean productIsUnique(String name, Product p) {
		try {
			...
			String query = "SELECT * FROM product WHERE name = '" + name
					+ "' AND visability=1 AND idproduct!='" + p.idproduct + "'";
